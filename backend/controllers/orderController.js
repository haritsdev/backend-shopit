...
exports.updateOrder = catchAsyncError(async (req, res, next) => {
  ...
  res.status(200).json({
    success: true,
    message: 'Sukses mengubah status pengiriman menjadi terkirim',
    data: { items: order.orderItems, status: order.orderStatus },
  });
});
...
exports.deleteOrder = catchAsyncError(async (req, res, next) => {
  ...
  res.status(200).json({ success: true, message: 'Order berhasil di hapus' });
});
